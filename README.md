...
Measurement sets can be split using casa, using the uvrange parameter of the split function. A ready-made docker for this can be found at rxastro/casa6. Otherwise, you can also use the casatools python library https://casadocs.readthedocs.io/en/latest/api/casatools.html. Check README.casatools for example code to perform the split.

## Results
For the raw data for our results, run the get_result.sh script in the results directory, which will perform the download. The results for the paper are visualized in a variety of jupyter notebooks. These are:
...
To run these notebooks, you will need to install rascil and its corresponding libraries, astropy, and the DeconvMultiStep library.
